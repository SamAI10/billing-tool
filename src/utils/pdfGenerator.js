...
export const generatePDF = (invoice, settings) => {
  ...
  // En-tête
  addText(settings.company.name, 10, headerStyle.fontSize, headerStyle.fontStyle);
  addText(settings.company.address, 10, normalStyle.fontSize);
  addText(settings.company.phone, 10, normalStyle.fontSize);
  addText(settings.company.email, 10, normalStyle.fontSize);
  ...
  const tva = total * (settings.invoiceSettings.taxRate / 100);
  ...
  doc.setFontSize(normalStyle.fontSize);
  doc.setFont('helvetica', 'normal');
  doc.text(settings.invoiceSettings.termsAndConditions, 10, yPosition);
